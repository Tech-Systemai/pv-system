...
const POLICY_TEMPLATES: Record<string, Clause[]> = {
  ...
  'Data Privacy': [
    ...
    { title: '2. Data Storage', content: 'Company data must only be stored on approved company systems and cloud platforms. Use of personal storage devices or unauthorised cloud services for company data is strictly prohibited.' },
    { title: '3. Data Sharing', content: 'Client and employee data must never be shared with third parties without explicit written authorisation from management. All third-party data-sharing arrangements must be documented and approved.' },
    { title: '4. Security Incidents', content: 'Any suspected data breach, unauthorised access, or security incident must be reported to management immediately. Failure to report a known or suspected breach may result in disciplinary action.' },
  ...
  'Safety': [
    ...
    { title: '2. Emergency Procedures', content: 'Employees must familiarise themselves with all emergency evacuation procedures, fire exits, and assembly points. Participation in emergency drills is mandatory.' },
    ...
    { title: '5. Compliance', content: 'All employees must comply with applicable occupational health and safety regulations. Wilful disregard for safety policies may result in disciplinary action up to and including termination.' },
  ],
  'Leave': [
    ...
    { title: '4. Parental Leave', content: 'Maternity and paternity leave entitlements are provided in accordance with applicable labour laws. Employees must provide advance notice and required documentation to HR.' },
  ...
  'Compensation': [
    ...
    { title: '2. Deductions', content: 'Authorised deductions from salary may include statutory contributions, advances, and penalty points as per the company\'s Policy Engine. All deductions will be reflected in the employee\'s payslip.' },
    ...
    { title: '5. Expense Reimbursement', content: 'Pre-approved work-related expenses will be reimbursed upon submission of receipts and an expense claim form. Unauthorised expenses will not be reimbursed.' },
  ],
  'General': [
...
function buildPolicyHtml(policy: Policy, clauses: Clause[], employeeName: string): string {
...
<div class="ack">
  ...
  <p style="font-size:12px;color:#4a5568;margin:8px 0 16px">I, <strong>${employeeName}</strong>, confirm that I have read, understood, and agree to comply with the <strong>${policy.title}</strong> policy as outlined above.</p>
  <div style="width:50%">
...
const STATUS_BADGE: Record<string, string> = {
  Draft: 'bdg bdg-gy', Active: 'bdg bdg-ok', Archived: 'bdg bdg-warn',
};
...
export default function PoliciesClient({
  ...
  const toggleAll = () =>
    setRecipients(prev => prev.length === users.length ? [] : users.map(u => u.id));

  const updateClause = (i: number, field: 'title' | 'content', val: string) =>
    setClauses(prev => prev.map((c, idx) => idx === i ? { ...c, [field]: val } : c));

  const addClause    = () => setClauses(prev => [...prev, { title: `${prev.length + 1}. New Clause`, content: '' }]);
  const removeClause = (i: number) => setClauses(prev => prev.filter((_, idx) => idx !== i));

  const loadAcks = async (policyId: string) => {
    const { data } = await dbOp('inbox_documents', 'select', undefined,
      { doc_ref_type: 'policy', doc_ref_id: policyId },
      'id,user_id,submitted_by_name,is_signed,signed_by,created_at');
    setAcks((data ?? []) as Ack[]);
  };

  const openCreate = () => {
    setEditing(null);
    setTitle(''); setCategory(CATEGORIES[0]); setStatus('Draft');
  ...
  const openEdit = (p: Policy) => {
    setEditing(p);
    setTitle(p.title); setCategory(p.category); setStatus(p.status);
  ...
  const openView = (p: Policy) => {
    ...
    setAcks(null);
    setView('preview');
    if (p.id) loadAcks(p.id);
  };

  const sendToInboxes = async (policy: Policy, targetIds: string[]) => {
    const resolvedClauses = parseClauses(policy.content).length
      ? parseClauses(policy.content)
      : POLICY_TEMPLATES[policy.category] ?? [];

    for (const uid of targetIds) {
      const emp  = users.find(u => u.id === uid);
      const name = emp?.name ?? 'Employee';
      const html = buildPolicyHtml(policy, resolvedClauses, name);
      await dbOp('inbox_documents', 'insert', {
        user_id: uid, sender_id: currentUserId,
        title: policy.title, subject: policy.title,
        content: `Please read and acknowledge the company policy: ${policy.title}`,
        type: 'Policy', sender: currentUserName,
        submitted_by_name: name,
        requires_signature: true, is_read: false, archived: false,
        html_content: html, doc_ref_type: 'policy',
        doc_ref_id: String(policy.id ?? ''),
      });
    }
  };

  const handleSave = async () => {
    if (!title.trim()) { setSaveError('Please enter a policy title.'); return; }
    setSaveError('');
    setIsSaving(true);
    ...
    const payload = {
      ...
      created_by: currentUserId, created_by_name: currentUserName,
    };
    ...
    if (recipients.length > 0 && saved) {
      await sendToInboxes(saved, recipients);
      showToast(`Policy saved and sent to ${recipients.length} employee${recipients.length !== 1 ? 's' : ''}`);
    } else {
      showToast(editing ? 'Policy updated' : 'Policy created');
    }
  ...
  const handleSendMore = async () => {
    ...
    await sendToInboxes(viewing, targets);
    setSendOpen(false); setSendTarget(''); setSendAll(false);
    setIsSending(false);
    showToast(`Policy sent to ${targets.length} employee${targets.length !== 1 ? 's' : ''}`);
    if (viewing.id) loadAcks(viewing.id);
  };
        ...
        <div style={{ background: '#fff', padding: '48px', maxWidth: 800, margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,.08)', borderRadius: 8, fontFamily: 'Inter, sans-serif' }}>
          ...
          <div style={{ marginTop: 48, borderTop: '2px solid #1a1f2e', paddingTop: 20 }}>
            ...
            <p style={{ fontSize: 12, color: '#4a5568', margin: '8px 0 16px', lineHeight: 1.7 }}>
              By signing in their inbox, the employee confirms they have read, understood, and agree to comply with this policy.
            </p>
        ...
        <div className="card no-print" style={{ marginTop: 24, maxWidth: 800, marginLeft: 'auto', marginRight: 'auto', overflow: 'hidden' }}>
          <div className="card-hdr">
            <div>
              ...
              <div className="card-sub">Employees who received this policy in their inbox</div>
            </div>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              {acks !== null && acknowledged > 0 && <span className="bdg bdg-ok">✓ {acknowledged} signed</span>}
              {acks !== null && pending > 0    && <span className="bdg bdg-warn">⏳ {pending} pending</span>}
              {isMgmt && (
                <button className="btn btn-acc btn-sm" onClick={() => setSendOpen(true)}>+ Send to More</button>
              )}
            </div>
          </div>
          ...
          {acks === null ? (
            ...
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--ink-4)', fontSize: 13 }}>
              ...
              <div style={{ fontWeight: 600, marginBottom: 4 }}>Not yet sent to any employee</div>
              {isMgmt && <div style={{ fontSize: 12 }}>Use "Send to More Employees" to distribute this policy.</div>}
            </div>
          ) : (
                      ...
                      <tr key={a.id}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <Avatar name={a.submitted_by_name || '?'} role={emp?.role ?? ''} />
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 13 }}>{a.submitted_by_name || '—'}</div>
                              {emp && <div style={{ fontSize: 11, color: 'var(--ink-4)', textTransform: 'capitalize' }}>{emp.role}</div>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td style={{ fontFamily: 'cursive', fontSize: 16, color: '#1a1f2e' }}>
                          {a.signed_by || <span style={{ fontFamily: 'inherit', fontSize: 11, color: 'var(--ink-4)' }}>—</span>}
                        </td>
                      </tr>
                    );
                  })}
        ...
        {sendOpen && mounted && createPortal(
          ...
          >
            <div style={{ background: '#fff', borderRadius: 16, width: 440, maxWidth: '100%', padding: 28, boxShadow: '0 24px 64px rgba(0,0,0,.22)' }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#1a1f2e', marginBottom: 6 }}>Send to Employee Inbox</div>
              <div style={{ fontSize: 12, color: '#6b7689', marginBottom: 20 }}>
                Select an employee or send to all. Each recipient will receive a signature request in their inbox.
              </div>
              <div className="pv-fld" style={{ marginBottom: 16 }}>
                <label>Send to</label>
                <select value={sendAll ? '__all__' : sendTarget} onChange={e => {
                  if (e.target.value === '__all__') { setSendAll(true); setSendTarget(''); }
                  else { setSendAll(false); setSendTarget(e.target.value); }
                }}>
                  <option value="">— Choose employee —</option>
                  <option value="__all__">📢 All Employees ({users.length})</option>
              ...
              {sendAll && (
                <div style={{ background: '#fef9c3', border: '1px solid #fde047', padding: '10px 14px', borderRadius: 8, fontSize: 12, color: '#854d0e', marginBottom: 16 }}>
                  ⚠️ This will send to all {users.length} employees.
                </div>
              )}
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button className="btn btn-sec" onClick={() => setSendOpen(false)}>Cancel</button>
                <button className="btn btn-acc" disabled={isSending || (!sendAll && !sendTarget)} onClick={handleSendMore}>
  ...
  if (view === 'create' && isMgmt) {
    ...
    const allSelected = recipients.length === users.length && users.length > 0;
    ...
    return (
      <div className="page-fade">
        <div className="briefing" style={{ marginBottom: 20 }}>
          <div>
            ...
            <div className="card-sub">
              {isEditing
                ? 'Update the policy details and clauses'
                : recipients.length > 0
                  ? `Will be sent to ${recipients.length} employee${recipients.length !== 1 ? 's' : ''} on save`
                  : 'Add details, clauses, and optionally assign recipients'}
            </div>
          </div>
            ...
            <button className="btn btn-acc" onClick={handleSave} disabled={isSaving || !title.trim()}>
              {isSaving
                ...
                : isEditing
                  ? 'Save Changes →'
                  : recipients.length > 0
                    ? `Create & Send to ${recipients.length} →`
                    : 'Create Policy →'}
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 20, alignItems: 'flex-start' }}>
          {/* Left column */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="card" style={{ padding: '20px 18px' }}>
              ...
              <div className="pv-fld">
                ...
                <input
                  ...
                  style={!title.trim() && saveError ? { borderColor: 'var(--err)' } : {}}
                />
              </div>
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              <div className="card-hdr">
                <div>
                  <div className="card-title">Assign Recipients</div>
                  <div className="card-sub">Employees will receive a signature request in their inbox</div>
                </div>
                <button
              ...
              <div style={{ maxHeight: 260, overflowY: 'auto', padding: '0 18px 14px' }}>
                {users.length === 0 ? (
                  <div style={{ padding: '20px 0', textAlign: 'center', fontSize: 12, color: 'var(--ink-4)' }}>No employees found.</div>
                ) : users.map(u => {
                  const checked = recipients.includes(u.id);
                  const hue = ROLE_HUES[u.role] ?? 220;
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        background: checked ? 'oklch(0.97 0.02 260)' : 'transparent',
                        borderRadius: 6, paddingLeft: checked ? 8 : 0, paddingRight: checked ? 8 : 0,
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--ink-4)', textTransform: 'capitalize' }}>{u.role}</div>
                      </div>
                      {checked && <span style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 600 }}>✓</span>}
                    </label>
                  );
                })}
              </div>
              {recipients.length > 0 && (
                <div style={{ padding: '10px 18px', borderTop: '1px solid var(--line)', background: 'var(--surface-2)', fontSize: 12, color: 'var(--ink-3)', fontWeight: 500 }}>
                  {recipients.length} employee{recipients.length !== 1 ? 's' : ''} selected — will receive inbox notification on save
                </div>
              )}
            </div>
          </div>
          ...
          <div className="card" style={{ padding: '20px 18px', position: 'sticky', top: 20 }}>
            ...
            {recipients.length > 0 && (
              <div style={{ marginTop: 14, padding: '10px 14px', background: 'oklch(0.96 0.03 260)', borderRadius: 8, border: '1px solid oklch(0.88 0.06 260)', fontSize: 12 }}>
                <div style={{ fontWeight: 700, color: 'var(--accent)', marginBottom: 6 }}>📨 Will send to:</div>
                {recipients.slice(0, 5).map(id => {
                  const u = users.find(u => u.id === id);
                  return u ? <div key={id} style={{ color: 'var(--ink-3)', marginBottom: 2 }}>• {u.name}</div> : null;
                })}
                {recipients.length > 5 && <div style={{ color: 'var(--ink-4)', marginTop: 2 }}>+{recipients.length - 5} more…</div>}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {policies.length === 0 ? (
          <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--ink-4)', fontSize: 13 }}>
            ...
            <div style={{ fontWeight: 600 }}>No policies on record</div>
              ...
              <tbody>
                {policies.map(p => (
                  <tr key={p.id}>
                    ...
                    <td style={{ fontSize: 12, color: 'var(--ink-4)' }}>{parseClauses(p.content).length}</td>
