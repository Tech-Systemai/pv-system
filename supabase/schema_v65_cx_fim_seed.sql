...
-- SOPs guarded with WHERE NOT EXISTS. Fault codes use ON CONFLICT (code) DO NOTHING.
-- Run once in the Supabase SQL Editor.

-- ─── SOPs ─────────────────────────────────────────────────────────────────────

INSERT INTO public.fim_sops (name, category, when_to_use, body, section)
SELECT name, category, when_to_use, body, section FROM (VALUES

('New Customer Onboarding', 'General',
...
7. Send the Post-Call Text Recap (SOP-GEN-6) immediately after the call.
...
('Disconnected or Missed Call', 'General',
...
('Stopped Paying Follow-Up', 'General',
 'Customer''s payment plan is active but payments have stopped or failed.',
 'WHEN: Customer has missed 1+ payment(s) with no response.
...
('Unclosed Sale Recovery', 'General',
...
1. Within 5 minutes of ending the call, send a personalized text:
   ...
   Feel free to reach out anytime — we''ve got you! 😊"
...
WHY: A written recap reinforces trust, reduces "I didn''t know" disputes, and gives the customer a reference they can revisit.',
 'cx'),

('ZOOM Impression Protocol', 'Impression',
...
('Veneer Fit & Look Assessment', 'Veneers',
 'Customer complains that veneers do not fit correctly or don''t look as expected.',
...
('Refund De-escalation', 'Sentiment',
...
('Fraud Concern Response', 'Sentiment',
 'Customer accuses the company of being fraudulent or a scam.',
 'WHEN: Customer says "this is a scam," "you''re fraudsters," or makes a similar accusation.
...
5. If they''re willing to continue: book a Zoom to demonstrate face-to-face professionalism.
...
('Marking a Case Unreachable', 'Operations',
 'Customer has not responded after 3+ attempts across multiple days.',
...
3. Send a final "we''re here if you need us" SMS before marking:
   "Hi [Name], we''ve tried reaching you a few times! We''re still here whenever you''re ready. Just reply or call us at your convenience."
...
('Condition Approval Process', 'Operations',
...
) AS v(name, category, when_to_use, body, section)
WHERE NOT EXISTS (
  SELECT 1 FROM public.fim_sops s WHERE s.name = v.name AND s.section = 'cx'
);


-- ─── Fault Codes ──────────────────────────────────────────────────────────────
-- NOTE: F-06 (putty), F-07 (shallow), F-08 (consecutive rejection), F-09 (dental work)
...
(105, 'Impression / Technical',
 ...
 '["Review any prior impression notes in CRM before calling","Offer to book a Zoom session at the customer''s earliest availability","Walk customer through kit contents and putty basics via phone if Zoom unavailable","Confirm appointment time via SMS and send calendar invite","Log appointment in CRM"]',
 'none', NULL,
...
(106, 'Impression / Technical',
 ...
 '["Send SMS: ''Hi [Name], this is [Agent] from Pioneers Veneers! Just tried calling to check in on your impression kit. Give us a ring when you get a chance!''","Attempt a second call the next business day","If no response after 3 attempts consider marking Unreachable (SOP-OPS-1)","Log all attempts with dates and method in CRM"]',
 'none', NULL,
...
(107, 'Impression / Technical',
 ...
 '["Acknowledge their concern: ''I understand, let''s talk about what''s worrying you''","Identify the real concern (fear of process, shipping delay, financial issue, doubt)","Offer a Zoom coaching session to address fear of the impression process","If financial: offer a payment plan adjustment or pause","Do NOT process any cancellation without Manager approval","Log conversation outcome and next step in CRM"]',
...
(108, 'Impression / Technical',
 ...
 '["Acknowledge the doubt: ''That''s a completely valid concern — let me explain how the kit works''","Walk them through the putty quality, lab certification, and fit guarantee","Offer to share before/after photos or customer testimonials","Book a Zoom to demonstrate the kit process live if needed","Log outcome in CRM"]',
 'none', NULL,
...
(109, 'Impression / Technical',
 ...
 '["Verify that the remake is approved in the system (check lab notes and CRM)","Confirm the correct shipping address with the customer","Place new kit order — flag as REMAKE in the order notes","Send tracking update SMS once shipped","Log kit dispatch date and order number in CRM","Book a follow-up Zoom session for impression coaching"]',
 'none', NULL,
 (SELECT id FROM public.fim_sops WHERE name = 'ZOOM Impression Protocol' AND section = 'cx')),

-- ── Category B: Veneers Fit ───────────────────────────────────────────────────

(401, 'Veneers Fit',
 'Veneers don''t fit right — general fit complaint.',
...
(402, 'Veneers Fit',
 ...
 '["Request 3-angle photos and a short video of the customer attempting to close their mouth","Confirm: is it upper veneers extending too far or overall bulk?","If bulk: submit lab adjustment request for reduced thickness","If new impression is required: follow new kit process (F-05 / code 109)","Provide ETA to customer and log in CRM"]',
...
(502, 'Veneers Look / Color',
 ...
 'Request photos and apply Veneer Fit & Look Assessment SOP.',
 '["Ask open-ended question: ''Can you describe what specifically doesn''t look right?''","Request 3 photos in natural daylight (front, left, right)","Identify specific issue: too white, wrong shape, gap, or length","Submit targeted lab correction request based on classification","If issue is subjective taste (not defect): offer a Zoom consultation with Manager","Log all findings and outcome in CRM"]',
...
(504, 'Veneers Look / Color',
 ...
 '["Review full case history: what shade was ordered, delivered, and corrected","Request new photos in natural lighting for current state","Do NOT submit another lab request without Manager approval","Present all findings and photos to Manager","Manager decides: third correction, refund credit, or alternative resolution","Communicate Manager''s decision to customer within 24h","Log all actions in CRM"]',
 'immediate', NULL,
 (SELECT id FROM public.fim_sops WHERE name = 'Refund De-escalation' AND section = 'cx')),
...
(602, 'Remakes',
 ...
 '["Acknowledge the customer''s ongoing frustration with genuine empathy","Request updated photos: front, left, right — in natural light","Do NOT promise another remake or refund without Manager involvement","Compile full case file: all remake notes, lab submissions, photos","Present to Manager for a formal resolution decision","Communicate outcome to the customer within 24h of Manager decision","Log all details in CRM"]',
 'immediate', NULL,
 (SELECT id FROM public.fim_sops WHERE name = 'Refund De-escalation' AND section = 'cx')),
...
(801, 'Delivery',
 ...
 '["Pull tracking and screenshot the Delivered confirmation","Ask customer to check all delivery points (mailbox, porch, neighbor, building office)","Verify the delivery address on file matches the customer''s address","If confirmed: file a carrier investigation (USPS/FedEx/UPS)","Report investigation case number to Manager","Await Manager approval before reshipping","Log all steps and carrier case number in CRM"]',
...
(802, 'Delivery',
 ...
 '["Sympathize: ''I''m really sorry to hear that — let''s look into this for you''","Confirm tracking shows Delivered and customer received a delivery notification","Advise customer to file a local police report if package had significant value","File a carrier theft/loss investigation on the carrier''s portal","Report to Manager with tracking evidence and police report reference if available","Await Manager decision on reship or partial resolution","Log all actions and case references in CRM"]',
...
(805, 'Delivery',
 ...
 '["Do not react defensively — acknowledge the concern fully","Apply Fraud Concern Response SOP","Share company legitimacy: verified reviews, real lab, BBB, money-back framing","Offer a Zoom to speak face-to-face and address every concern","If chargeback is already filed: escalate to Manager immediately","Log every statement and outcome verbatim in CRM"]',
 'conditional', 'If a chargeback has been filed — escalate to Manager immediately.',
 (SELECT id FROM public.fim_sops WHERE name = 'Fraud Concern Response' AND section = 'cx')),

-- ── Category G: Follow-Ups ────────────────────────────────────────────────────
...
(902, 'Follow-Ups',
 ...
 '["Review when the last contact was made and what the last open action was","Call immediately — prioritize this case in today''s queue","Acknowledge to the customer that you''re checking in (don''t highlight the gap unless they bring it up)","Resolve any outstanding concern or set next steps","Log reason for delayed follow-up internally and take corrective action","Flag in CRM if there is a pattern of missed follow-ups on this case"]',
 'none', NULL, NULL),
...
(904, 'Follow-Ups',
 ...
 '["Check Stripe for exact payment failure date and amount","Apply Stopped Paying Follow-Up SOP","Offer flexibility: payment link, rescheduled date, or partial payment","If production is in progress: inform Manager whether to pause","Log payment status and customer''s stated reason in CRM","If 3+ failed attempts: escalate to Manager"]',
 'conditional', '3+ contact attempts fail — escalate to Manager.',
...
(905, 'Follow-Ups',
 ...
 'Confirm the patient''s details and adjust the CRM record accordingly.',
 '["Confirm who the ordering contact is vs. who the veneers are for","Update CRM with patient name, relationship, and relevant dental notes","Clarify communication preference: contact the buyer or the patient directly?","Ensure impression kit is addressed to the patient''s location","Note any dental condition specifics for the actual patient","Log all updates in CRM"]',
 'none', NULL, NULL),

-- ── Category H: Dentistry Special ────────────────────────────────────────────
...
-- ── Category I: New / Returning Customers ─────────────────────────────────────

(1101, 'New / Returning',
 'Significant communication delay occurred on the company''s side.',
 'Acknowledge the delay, apologize, and re-engage with urgency.',
 '["Review the communication gap: when was the last contact and why was there a delay?","Open the call with a direct apology: ''I want to apologize for the delay in getting back to you — that''s not the experience you should have''","Offer a gesture of goodwill if appropriate (free adhesive, priority handling)","Address their current status and concern immediately","Log reason for delay and corrective action in CRM","Flag internally if the delay was a systemic or team issue"]',
...
(1201, 'Happy Customers',
 ...
 '["Congratulate the customer on their new smile","Ask: ''Would you be open to leaving us a review? It would mean the world to us''","Send a Google Review or Trustpilot link via SMS","Ask: ''Do you know anyone who might also be interested in Pioneers Veneers?''","Log happiness status, review request sent, and any referral leads in CRM"]',
 'none', NULL, NULL),
...
(1202, 'Happy Customers',
 ...
 '["Acknowledge the slight looseness: ''That''s a normal adjustment for some customers''","Ship dental adhesive immediately if they don''t already have it","Walk customer through correct adhesive application","Schedule a 2-week follow-up to confirm fit improvement","If looseness persists after adhesive trial: assess for a remake (code 403)","Log adhesive shipment and follow-up date in CRM"]',
 'none', NULL,
 (SELECT id FROM public.fim_sops WHERE name = 'Veneer Fit & Look Assessment' AND section = 'cx')),
...
(1203, 'Happy Customers',
 ...
 'conditional', '3+ contact attempts fail — escalate to Manager for collections.',
 (SELECT id FROM public.fim_sops WHERE name = 'Stopped Paying Follow-Up' AND section = 'cx'))
